...
const handRanker = (hand: HandData): HandData => {
  ...
  const handTypes: HandType[] = [
    ...
    {
      type: "twoPair",
      condition: counts.filter((count) => count === 2).length === 2,
    },
    {
      type: "onePair",
      condition: counts.filter((count) => count === 2).length === 1,
    },
  ...
  const handTypes: HandType[] = [
    {
      type: "fiveOfAKind",
      condition:
        counts.some((count) => count + numberOfJokers >= 5) ||
        numberOfJokers === 5,
    },
    {
      type: "fourOfAKind",
      condition: counts.some((count) => count + numberOfJokers >= 4),
    },
    {
      type: "fullHouse",
      condition:
    ...
    {
      type: "threeOfAKind",
      condition: counts.some((count) => count + numberOfJokers >= 3),
    },
    {
      type: "twoPair",
      condition:
...
const rankHands = (hands: HandData[], partTwo: boolean = false): HandData[] => {
  ...
  const initialHandRanks = hands
    .map(partTwo ? jokerHandRanker : handRanker)
  ...
  while (i < initialHandRanks.length) {
    let j = i;
    while (
      j < initialHandRanks.length &&
      initialHandRanks[j].rank === initialHandRanks[i].rank
    ) {
      j++;
    }
  ...
  const rankedHands = initialHandRanks.map((hand, index) => ({
    ...hand,
    rank: index + 1,
  }));
  return rankedHands;
...
export function solve(content: string) {
  ...
  const totalWinnings = ranks1.reduce(
    (total, hand) => total + hand.bid * hand.rank,
    0,
  );
  const jokerWinnings = ranks2.reduce(
    (total, hand) => total + hand.bid * hand.rank,
    0,
  );
